...
# ─────────────────────────────────────────────────────────────
st.markdown('<div class="section-header" id="performance-overview">🎯 Performance Overview</div>', unsafe_allow_html=True)

ic = bt1["initial_capital"]   if bt1 else 100000
fc = bt1["final_capital"]     if bt1 else ic
pnl1 = bt1["net_pnl"]         if bt1 else 0
ret1 = bt1["total_return_pct"] if bt1 else 0
pnl2 = bt2["net_pnl"]         if bt2 else 0
ret2 = bt2["total_return_pct"] if bt2 else 0
grand_final = (fc + pnl2) if bt2 else fc
grand_pnl   = grand_final - ic
...
with col1:
    st.markdown(metric_html("Initial Capital", f"₹{ic:,.0f}", "purple"), unsafe_allow_html=True)
with col2:
    sign = "green" if pnl1 >= 0 else "red"
    st.markdown(metric_html("Jan–Apr 2026 P&L", f"₹{pnl1:,.0f}", sign), unsafe_allow_html=True)
with col3:
    sign = "green" if ret1 >= 0 else "red"
    st.markdown(metric_html("Jan–Apr Return", f"{ret1:+.1f}%", sign), unsafe_allow_html=True)
with col4:
    sign = "green" if pnl2 >= 0 else "red"
    st.markdown(metric_html("30-Day P&L (Live)", f"₹{pnl2:,.0f}", sign), unsafe_allow_html=True)
with col5:
    sign = "green" if ret2 >= 0 else "red"
    st.markdown(metric_html("30-Day Return", f"{ret2:+.1f}%", sign), unsafe_allow_html=True)
with col6:
    sign = "green" if grand_pnl >= 0 else "red"
    st.markdown(metric_html("Total Net P&L", f"₹{grand_pnl:,.0f}", sign), unsafe_allow_html=True)
...
with col1:
    trades1 = bt1["total_trades"] if bt1 else 0
    st.markdown(metric_html("Trades (Jan–Apr)", trades1, "blue"), unsafe_allow_html=True)
with col2:
    wr1 = bt1["win_rate_pct"] if bt1 else 0
    c = "green" if wr1 >= 50 else "gold"
    st.markdown(metric_html("Win Rate (Jan–Apr)", f"{wr1:.1f}%", c), unsafe_allow_html=True)
with col3:
    pf1 = bt1["profit_factor"] if bt1 else 0
    c = "green" if pf1 >= 1 else "red"
    st.markdown(metric_html("Profit Factor", pf1, c), unsafe_allow_html=True)
with col4:
    dd1 = bt1["max_drawdown_pct"] if bt1 else 0
    st.markdown(metric_html("Max Drawdown", f"-{dd1:.1f}%", "red"), unsafe_allow_html=True)
with col5:
    wr2 = bt2["win_rate_pct"] if bt2 else 0
    c = "green" if wr2 >= 50 else "gold"
    st.markdown(metric_html("Win Rate (Live 30d)", f"{wr2:.1f}%", c), unsafe_allow_html=True)
...
with tab3:
    ...
    if bt_sel:
        ...
        if not t_df.empty:
            ...
            show_cols = ["entry_time","exit_time","direction","signal","confidence","entry_price","exit_price","pnl_rs","pnl_pct","exit_reason","capital_after"]
            col_names = {"entry_time":"Entry","exit_time":"Exit Time","direction":"Dir","signal":"Signal","confidence":"Conf",
                         "entry_price":"Entry ₹","exit_price":"Exit ₹","pnl_rs":"P&L (₹)","pnl_pct":"P&L%","exit_reason":"Reason","capital_after":"Capital ₹"}
...
with tab4:
    ...
    if bt2:
        ...
        fig8.update_layout(height=500, showlegend=False, **PLOTLY_LAYOUT)
        ...
        if not t2_df.empty:
            ...
            show = ["entry_time","exit_time","direction","signal","entry_price","exit_price","pnl_rs","exit_reason"]
