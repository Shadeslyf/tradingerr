...
def main():
    ...
    # 1-Year Slice
    df_slice = df_raw[(df_raw["timestamp"].dt.date >= pd.to_datetime("2025-04-23").date()) & (df_raw["timestamp"].dt.date < pd.to_datetime("2026-04-30").date())].copy()
    
    logger.info("Building Features...")
    df_feat = FeaturePipeline.generate_features(df_slice)
    
    # We must align df_slice to df_feat because FeaturePipeline drops initial rows
    ...
    for name, path, is_v3, is_v4 in models:
        logger.info(f"Running House Money Backtest for {name.upper()}...")
        res = run_backtest(df_slice, df_feat, path, initial_capital=100000.0, label=f"{name.upper()} (House Money)", is_v3=is_v3, is_v4=is_v4, house_money_mode=True)
        
        out_path = f"data/backtest_results/backtest_jan_apr_2026_{name}_hm.json"
