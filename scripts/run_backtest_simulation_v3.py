...
# CONFIG
# ─────────────────────────────────────────────────────────────
MODEL_PATH       = "models/walk_forward_v3/best_model.joblib"
RAW_2Y_CSV       = "data/Nifty50_CrossAsset_Merged.csv"
REAL_30D_CSV     = "data/real_nifty_30d.csv"
OUT_JSON_MAIN    = "data/backtest_results/backtest_jan_apr_2026_v3.json"
OUT_JSON_LIVE    = "data/backtest_results/backtest_30d_live_v3.json"
...
def run_backtest(df_feat: pd.DataFrame, model, initial_capital: float, label: str) -> dict:
    ...
    while i < n - HOLD_BARS:
        ...
        # V3 ATR Trailing Stop Logic (Chandelier Exit)
        ATR_MULT = 2.5
...
def main():
    ...
    # ── Combined summary ─────────────────────────────────────
    combined = {
        "backtest_jan_apr_2026_v3": result_1 if not feats_1.empty else {},
        "backtest_30d_live_v3":     result_2 if not feats_2.empty else {},
        "generated_at":          datetime.now().isoformat(),
    }
    
    # We will write out these specific V3 outputs so dashboard can read them independently
    combined_path = os.path.join(RESULTS_DIR, "combined_results_v3.json")
    with open(combined_path, "w") as f:
        json.dump(combined, f, indent=2)
    logger.info(f"V3 combined results saved: {combined_path}")
